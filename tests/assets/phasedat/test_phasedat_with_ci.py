...
class TestPhaseDatCI:
    ...
    def test_phasedat_adev(self):
        ...
        for (d,t, n, s32) in zip(devs, taus, ns, s32_rows):
            ...
            edf2 = allantoolkit.ci.edf_greenhall(
                alpha=0, d=2, m=t, N=len(phase),
                overlapping = False, modified=False )

            (lo, hi) = allantoolkit.ci.confidence_interval(dev=d, edf=edf2)
    ...
    def test_phasedat_hdev(self):
        ...
        for (d,t, n) in zip(devs, taus, ns):
            ...
            edf2 = allantoolkit.ci.edf_greenhall(
                alpha=0, d=3, m=int(t), N=len(phase),
                overlapping=False, modified=False)
    ...
    def test_phasedat_tdev(self):
        ...
        for (d, t, n) in zip(devs, taus, ns):
            ...
            (lo, hi) = allantoolkit.ci.confidence_interval(dev=mdev, edf=edf2)
