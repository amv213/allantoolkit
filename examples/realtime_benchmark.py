...
for n_pts in [2,4,8,16,32,64,128]:
    ...
    #print "done"
    print('%s\t%d\t%.3f\t%.3f'% ('TDEV',n_afs,1e3*t/n_add,1e6*t/(n_afs*n_add)) )

print('DEV\tn_AFs\tms/point\tus/(AF*point)')
for n_pts in [2,4,8,16,32,64,128]:
    (n_afs,t,n_add)=bench(4, n_pts, function=at.realtime.ohdev_realtime)
    #print "done"
    print('%s\t%d\t%.3f\t%.3f'% ('OHDEV',n_afs,1e3*t/n_add,1e6*t/(n_afs*n_add)) )

print('DEV\tn_AFs\tms/point\tus/(AF*point)')
for n_pts in [2,4,8,16,32,64,128]:
    (n_afs,t,n_add)=bench(4, n_pts, function=at.realtime.oadev_realtime)
    #print "done"
    print('%s\t%d\t%.3f\t%.3f'% ('OADEV',n_afs,1e3*t/n_add,1e6*t/(n_afs*n_add)) )
