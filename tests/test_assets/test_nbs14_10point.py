...
class TestNBS14_10Point:
    ...
    def generic_test(function, ref_devs):
        ...
        out1 = function(nbs14_phase, rate=1.0, data_type='phase', taus=taus)
        out2 = function(nbs14_f, rate=1.0, data_type="freq", taus=taus)

        assert(check_devs(out1.devs[0], ref_devs[0]))  # tau=1 from phase data
        assert(check_devs(out1.devs[1], ref_devs[1]))  # tau=2 from phase data
        assert(check_devs(out2.devs[0], ref_devs[0]))  # tau=1 from frequency
        # data
        assert(check_devs(out2.devs[1], ref_devs[1]))  # tau=2 from frequency
        # data
